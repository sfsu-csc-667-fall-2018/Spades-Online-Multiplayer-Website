...
var MESSAGE_SEND = 'mesage-send';

module.exports = { USER_JOINED: USER_JOINED, MESSAGE_SEND: MESSAGE_SEND };
...
var writeMessage = function writeMessage(message) {
...
var messageElement = function messageElement(_ref) {
  ...
  $('<div>', { class: 'message' }).text(message).prepend(timetampElement(timestamp), userElement(user));
};
...
var initializeSocket = function initializeSocket() {
  ...
  socket.on(_constants.USER_JOINED, userJoined);
  socket.on(_constants.MESSAGE_SEND, messageReceived);
};
...
$(document).ready(function () {
  ...
  $('#initial-form button').click(function (event) {
    ...
    intializeSocket();
    socket.emit(_constants.USER_JOINED, { user: user, timestamp: Date.now() });
  ...
  $('#chat-area button').click(function (event) {
    ...
    socket.emit(_constants.MESSAGE_SEND, { user: user, timestamp: Date.now(), message: message });
  });
});

},{"./constants":2,"moment":1}]},{},[3]);
