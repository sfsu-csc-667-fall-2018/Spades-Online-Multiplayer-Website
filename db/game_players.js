...
const addPlayer = (game_id, player_id) => {
  return new Promise(function (resolve, reject) {
    getPositions(game_id)
      .then((positions) => {
        const emptyPos = getEmptyPostion(positions)
        if (emptyPos != null) {
          const team = getTeam(emptyPos);
      ...
      .catch((error) => {
        reject("Failed to add player to games_players: ", error)
      })
    })
    .catch((error) => {
      reject("Failed to get positions", error)
    })
};
